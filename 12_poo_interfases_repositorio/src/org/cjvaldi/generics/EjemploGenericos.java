package org.cjvaldi.generics;

import org.cjvaldi.poointerfases.modelo.Cliente;
import org.cjvaldi.poointerfases.modelo.ClientePremium;
...
public class EjemploGenericos {
    public static void main(String[] args) {
        ...
        clientes.add(new Cliente("Cristian", "Jacson"));

        Cliente cristian = clientes.iterator().next();

        Cliente[] clientesArreglos = {new Cliente("Luci", "Martinez"),
                new Cliente("Cristian", "Jacson")};
        Integer[] enterosArreglo = {1, 2, 3};

        List<Cliente> clienteLista = fromArrayToList(clientesArreglos);
        List<Integer> enterosLista = fromArrayToList(enterosArreglo);

        clienteLista.forEach(System.out::println);
        enterosLista.forEach(System.out::println);

        List<String> nombres = fromArrayToList(new String[]{"Cristian", "Andres", "Luci", "Bea", "Jhon"},
                enterosArreglo);
